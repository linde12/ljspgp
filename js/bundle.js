...
var RSA = function () {
    ...
    var API = {
         ...
         */
        computeE: function (totient) {
            for (var i = bignum('2'); i.lt(totient); i.add(1)) {
                if (totient.mod(i) !== 0 && i.probPrime() === true) {
                    return i;
                }
            }
        },
        ...
        encrypt: function (message, publicKey) {
            var chiper = new Uint32Array(message.length);
            for (var i = 0; i < message.length; i++) {
                chiper[i] = Math.pow(message.charCodeAt(i), publicKey.e) % publicKey.product;
            }
        ...
        decrypt: function (message, privateKey) {
            var plain = "";
            for (var i = 0; i < message.length; i++) {
                plain += String.fromCharCode(Math.pow(message[i], privateKey.d) % privateKey.product);
            }
...
var rsa = new RSA();
var keypair = rsa.createRSA(bignum.prime(4), bignum.prime(4));

//var c = rsa.encrypt("A", keypair.publicKey);
//console.log(rsa.decrypt(c, keypair.privateKey));
...
},{"bignum":2}],2:[function(require,module,exports){
(function (Buffer){
try {
